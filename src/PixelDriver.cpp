...
void PixelDriver::start() {
        ...
        xTaskCreatePinnedToCore([](void *parameter) { static_cast<PixelDriver *>(parameter)->pixelTask(); },
                                "pixelTask", 4096, this, 1, NULL, 0);

}

    void PixelDriver::pixelTask() {
        Serial.printf("pixelTask: started on core %d\n", xPortGetCoreID());
    ...
    void PixelDriver::interfaceTask() {
        Serial.printf("interfaceTask started on core %d\n", xPortGetCoreID());
